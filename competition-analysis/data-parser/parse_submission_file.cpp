...
void load_file(const std::string, std::vector< std::vector<double> > &, std::vector< std::vector<double> > &, const int &);
void load_file_keep_last_archive(const std::string, std::vector< std::vector<double> > &, std::vector< std::vector<double> > &, const int &);
...
int main(int argc, char **argv)
{
	...
	Array3D results;
	Array2D PR, SR, pop, costs, all_dists;
	...
	for (int index = 1; index <= problems; ++index) {
		...
		for (int irun = 0; irun < noruns; ++irun) {
			...
			string tmpf = respath + "problem"  + int_to_string_with_leading_zeros(index,3) 
								+ "run" + int_to_string_with_leading_zeros(irun+1,3) + ".dat";

			/* Load file and keep the last archive based on GECCO 2017 submission format */
			load_file_keep_last_archive(tmpf, pop, costs, dim);
			
			// re-evaluate solutions
			re_evaluate_solutions(fitnesses, pop, pFunc);
			...
			// Create data file.
			create_data_file(p, index, irun, pop, fitnesses, costs, goptima, pFunc, all_dists, results, noacc, acclevels);
		}
		cout << endl;
